export const UrlConstant = {
    Ip: "http://4e08d8b7.ngrok.io/api/1/ims/",
    login: "users/login"
};


export const dropDownConstants = {
    unit: [{ text: 'Ltr', value: 'Ltr' }, { text: 'Mtr', value: 'Mtr' }],
    status: [{ text: 'Active', value: 'Active' }, { text: 'InActive', value: 'InActive' }],
    category: [{ text: 'category1', value: 'category1' }, { text: 'category2', value: 'category2' }],
    make: [{ text: 'make1', value: 'make1' }, { text: 'make2', value: 'make2' }],
    origin: [{ text: 'Indian', value: 'indian' }, { text: 'Mexican', value: 'mexican' }, { text: 'Italian', value: 'italian' }],
    difficultylLevel: [{ text: 'Easy', value: 'Easy' }, { text: 'Difficult', value: 'difficult' }, { text: 'Tough', value: 'tough' }],
    recipeCategory: [{ text: 'Veg', value: 'Veg' }, { text: 'Jain', value: 'Jain' }, { text: 'Vegan', value: 'Vegan' },{ text: 'Non Veg', value: 'Non Veg' }],
    recipeType: [{ text: 'Snacks', value: 'snacks' }, { text: 'Main Dish', value: 'main dish' }, { text: 'Rice', value: 'rice' }],
    ingredient: [{ text: 'Onion', value: 'Onion' }, { text: 'Tomato', value: 'Tomato' }, { text: 'Potato', value: 'Potato' }],
}

