      ...
      txn.executeSql(
        ...
        function (txn, res) {
          ...
          if (res.rows.length === 0) {
            ...
            txn.executeSql(
              'CREATE TABLE IF NOT EXISTS table_user(id INTEGER PRIMARY KEY AUTOINCREMENT, userid VARCHAR(255),emailid VARCHAR(255), account_name  VARCHAR(255), password  VARCHAR(255),is_active INTEGER,address VARCHAR(255),longitude REAL,latitude REAL,code VARCHAR(255))',
              []
            );
          } else {
            console.log('user length `:', res.rows.length);
  ...
  const createNewUser = async () => {
    ...
    db.transaction(function (tx) {
      ...
      tx.executeSql(
        ...
        [responsedata.account_id, username, accountname, password,1,'',77.5946,12.9716,''],
        (tx, results) => {
